...
WEIGHTS_CALCULATETOTALSPENT = [1.0]
INPUTS_CALCULATETOTALSPENT  = ["userId"]

@app.post('/compute/calculateTotalSpent')
def compute(inp: ComputeIn):
    xs = [getattr(inp, k, 0.0) for k in INPUTS_CALCULATETOTALSPENT]
    ws = WEIGHTS_CALCULATETOTALSPENT
    if len(xs) != len(ws):
