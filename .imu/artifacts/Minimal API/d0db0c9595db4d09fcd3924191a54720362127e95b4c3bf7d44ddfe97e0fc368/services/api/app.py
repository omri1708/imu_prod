...
WEIGHTS_CALCULATETOTALORDERAMOUNT = [1.0]
INPUTS_CALCULATETOTALORDERAMOUNT  = ["orderId"]

@app.post('/compute/calculateTotalOrderAmount')
def compute(inp: ComputeIn):
    xs = [getattr(inp, k, 0.0) for k in INPUTS_CALCULATETOTALORDERAMOUNT]
    ws = WEIGHTS_CALCULATETOTALORDERAMOUNT
    if len(xs) != len(ws):
