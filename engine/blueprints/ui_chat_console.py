# PATH: engine/blueprints/ui_chat_console.py
# -*- coding: utf-8 -*-
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--text);font-family:system-ui,Segoe UI,Helvetica,Arial}
    ...
    .user{background:#132133;border:1px solid #1f2c3a}
    .bot{background:#0f141b;border:1px solid #1a2330}
    ...
    a, a:visited{color:#74d7ff}
    code{background:#0b1016;border:1px solid #1a2330;border-radius:6px;padding:2px 4px}
  </style>
</head>
<body>
<header>
  <strong>IMU Chat</strong>
  <label>API Base:
    <input id="apiBase" value="http://localhost:8000" style="width:240px">
  </label>
  <label>User:
    <input id="userId" value="u1" style="width:100px">
  </label>
  <label>Mode:
    <select id="mode"><option value="live" selected>live</option><option value="poc">poc</option><option value="prod">prod</option></select>
  </label>
  <button id="resetBtn">אפס שיחה</button>
</header>

<div id="main">
  <aside id="side">
    <h3>אפשרויות בנייה</h3>
    <div class="kv">
...
const apiBase=$('#apiBase'), userId=$('#userId'), mode=$('#mode');
const proceed=$('#proceed'), autobuild=$('#autobuild'), persist=$('#persist');
const emit_ci=$('#emit_ci'), emit_iac=$('#emit_iac'), autodeploy=$('#autodeploy');
...
async function send(){
  ...
  addMsg(msg,"user"); promptEl.value="";
  const body={ user_id:userId.value||"u1", message:msg, mode:mode.value||"live", proceed:proceed.checked, autobuild:autobuild.checked };
  if(persist.checked) body.persist="./out";
  if(emit_ci.checked) body.emit_ci=true;
  if(emit_iac.checked) body.emit_iac=true;
  if(autodeploy.checked) body.autodeploy=true;

  const t0=performance.now();
  try{
    const r=await fetch(apiBase.value.replace(/\/$/,"")+"/chat/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
    const j=await r.json(); const t1=performance.now();
    const meta=[]; meta.push(`⏱ ${(t1-t0).toFixed(0)}ms`);
    if(j.extra?.build){const b=j.extra.build; meta.push(`build: ${b.ok?"OK":"FAIL"} | compile=${b.compile_rc} test=${b.test_rc}`)}
    ...
    $('#lastMeta').innerHTML=meta.join(" · ")||"—";
    addMsg(j.text||"(no text)","bot",meta.join(" · "));
  }catch(e){ addMsg("שגיאת רשת/‏API: "+e,"bot"); }
}
$('#sendBtn').onclick=send;
promptEl.addEventListener('keydown', e=>{ if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send()} });
$('#resetBtn').onclick=async()=>{ try{await fetch(apiBase.value.replace(/\/$/,"")+"/chat/reset",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({user_id:userId.value||"u1"})}); addMsg("ההקשר אופס.","bot"); $('#lastMeta').textContent="—"}catch(e){addMsg("שגיאה באיפוס: "+e,"bot");}};
</script>
</body>
</html>
"""
...
NGINX_CONF = """server {
  ...
  # פרוקסי לכל נתיבי /chat/ ל-API (רץ כשירות 'api' ברשת הדוקר)
  location /chat/ {
...
DOCKERFILE_UI = """# docker/ui/Dockerfile (chat console)
FROM nginx:alpine
...
def generate(spec: Dict[str, Any]) -> Dict[str, bytes]:
    return {
        "ui/chat.html": CHAT_HTML.encode("utf-8"),
        "docker/ui/nginx.conf": NGINX_CONF.encode("utf-8"),
        "docker/ui/Dockerfile": DOCKERFILE_UI.encode("utf-8"),
    }
