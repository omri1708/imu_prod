...
class LLMGateway:
    ...
    def chat(
        ...
        *,
        ...
        i, arm = self._ucb.select()
        model_name = arm["name"]
        temperature = arm["temp"]  # דריסה עדינה לטמפ'
        start = time.time()
    ...
    def structured(
        ...
        *,
        ...
        raw = ""
        # === BEGIN: Cache JSON wrapper ===
        # meta עבור cache + near-hit
        meta = {
            "model": self.oa_json_model if _HAS_OPENAI else (self.claude_model if _HAS_ANTHROPIC else "local"),
            "system_v": "1", "template_v": "1",
            "tools": [], "ctx_ids": [], "persona_v": "1", "policy_v": "1",
            "ttl_s": 3600, "allow_near_hit": True, "fresh_only": False
        }
        # משתמשים במחרוזת prompt/json כדי ליצור key יציב
        user_text = (content.get("prompt") if isinstance(content, dict) else None) or ""
        prompt_for_cache = {"user_text": user_text, "meta": meta}

        def _llm_json_fn(_: Dict[str, Any]) -> Dict[str, Any]:
            # חייב להחזיר JSON כטקסט (נעטוף לתוכן)
            if _HAS_OPENAI:
                raw_local = self._openai_chat(msgs, json_mode=True, temperature=temperature)
            elif _HAS_ANTHROPIC:
                # AnthropIc אצלך מחזיר JSON בטקסט רגיל — שמרנו התנהגות קודמת
                raw_local = self._anthropic_chat(msgs, json_mode=False, temperature=temperature)
            else:
                if require_grounding:
                    return {"content": json.dumps({"ok": False, "error": "llm_unavailable_for_grounded_call"})}
                return {"content": json.dumps(self._structured_local(intent, str(content.get("prompt", ""))))}
            return {"content": raw_local, "meta": {"json_mode": True}}
